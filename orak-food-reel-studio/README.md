...
## 11-2. 제작한 영상에 AI 음성 입히기 (외부 영상)

다른 도구로 만든 영상(mp4·mov 등)에 나레이션만 AI 목소리로 넣고 싶을 때 씁니다.

1. 🎞 **외부 영상 AI 음성** 메뉴 (또는 🎬 제작중 화면의 **[➕ 제작한 영상 불러와 AI 음성 입히기]**)
2. 영상 파일 고르기 — 설치형 앱은 파일 창이 뜨고, 폴더 실행(start.bat)은 전체 경로를 붙여넣습니다
3. 나레이션 입력 → 목소리·모델·속도 선택 → 기존 소리 **끄기** / **작게 섞기**
4. **[🎬 최종 MP4 만들기]** → 결과 파일 검증까지 끝나면 `완성영상/날짜_외부영상_이름/최종_AI음성.mp4`

- 원본 영상은 읽기만 하고 바꾸지 않습니다. 음성이 영상보다 길면 마지막 장면을 멈춘 채 이어 붙입니다(음성을 자르지 않음).
- 여기서 고른 목소리는 이번 영상에만 쓰입니다. **[기본 목소리로 저장]**에 표시한 경우에만 전체 설정이 바뀝니다.
- 연습 모드에서는 실제 목소리 대신 샘플 톤이 들어갑니다. 영상 속 말을 자동으로 받아 적는 기능은 아직 없습니다.
- 🎬 제작중 화면의 **[📁 저장 폴더 열기]**로 각 작업의 저장 폴더를 바로 열 수 있습니다(설치형 앱).

## 11-3. 주제 고르기 · 인트로/아웃트로 · 설정 정리

- **주제 고르기**: ✨ 오늘의 릴스에서 4대 분류(일상·관계 / 동네·맛집 / 생활 취미 / 오락 브랜딩)를 누르면 세부 주제를 추천합니다.
  연습 모드나 키가 없을 때는 예시 주제, Claude 키가 있으면 Claude, 설정 → AI 에 제미나이 모델 이름을 넣으면 제미나이(이미지 생성의 Gemini 키 공용)가 추천합니다.
  주제를 고르면 콘텐츠 유형에 채워진 채로 돌아옵니다. 영상 스타일은 **맛집 사건 파일** / **일반 맛집 영상** 중 고릅니다.
- **인트로 · 아웃트로**: 설정 → 🎞 인트로 · 아웃트로에서 로고·배너 그림을 올리면 모든 완성 영상 맨 앞·맨 뒤에 붙습니다(붙인 만큼 길어짐).
  붙이기 전 영상은 같은 폴더에 `reel.raw.mp4` 로 남습니다. 맛집 릴스·외부 영상 각각 켜고 끌 수 있습니다.
- **설정 화면**은 구역마다 제목을 눌러 접고 펼칩니다. 🎬 영상·자막 안에 **영상 생성 AI(클링 AI) 에이전트·모델·API 키** 칸이 있습니다 — 지금은 저장만 되고 제작에는 아직 쓰이지 않습니다.
- 왼쪽 메뉴에서 **맛집 DB**는 🔍 릴스 벤치마킹 아래로 들어갔고, 🥟 만두탐정 오락이 화면은 위쪽 띠(화면의 1/10)로 줄고 자세한 내용은 펼쳐 봅니다.
