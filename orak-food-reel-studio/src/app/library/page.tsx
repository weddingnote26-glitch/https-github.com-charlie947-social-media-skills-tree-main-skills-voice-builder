      ...
      <div className="flex gap-2">
        {FILTERS.map((f) => (
          <button key={f} onClick={() => setFilter(f)}
            className={`rounded-full px-4 py-1.5 text-sm font-bold border-2 ${filter === f ? "border-[#E86A3A] bg-[#FDEDE5] text-[#E86A3A]" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
            {f}
          </button>
        ))}
      </div>
        ...
        <div className="grid grid-cols-5 gap-4">
          {(data?.reels ?? []).map((r) => (
            <Link key={r.id} href={`/reel/${r.id}`} className="rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition">
              ...
              <div className="p-2.5">
                <div className="flex items-center gap-1"><StatusBadge status={r.status} />{r.case_number && <span className="text-[10px] font-bold text-[#E86A3A]">#{String(r.case_number).padStart(3, "0")}</span>}</div>
                <div className="text-sm font-bold truncate mt-1">{r.title || "제목 없음"}</div>
                <div className="text-xs text-gray-400">{r.planned_date} · {r.duration_sec ? `${Math.round(r.duration_sec)}초` : "-"}</div>
              </div>
            </Link>
          ))}
          {(data?.reels?.length ?? 0) === 0 && <div className="col-span-5 text-center text-gray-400 py-14">해당 상태의 콘텐츠가 없습니다.</div>}
        </div>
      </Card>
    </div>
  );
