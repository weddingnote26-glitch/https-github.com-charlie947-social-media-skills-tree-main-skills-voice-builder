    ...
    <div className="space-y-6">
      <header className="flex items-end justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">오늘도 맛있는 사건을 찾아 신림 골목을 조사합니다.</p>
        </div>
        <Link href="/today" className="btn-primary">✨ 오늘의 릴스 만들기</Link>
      </header>

      {health && !health.settings.wizardDone && (
        <Link href="/wizard" className="block card p-4 border-[#E86A3A] bg-[#FDEDE5] font-bold text-[#E86A3A]">
        ...
        <div className="grid grid-cols-6 gap-3">
          {(data?.weekDays ?? []).map((d) => (
            <div key={d.date} className="rounded-xl border border-gray-200 p-3 min-h-24">
              <div className="text-sm font-extrabold mb-2">{d.weekday} <span className="text-gray-400 font-normal">{d.date.slice(5)}</span></div>
              {d.reels.length === 0 && <div className="text-2xl text-gray-300">○</div>}
              {d.reels.map((r) => (
                <Link key={r.id} href={`/reel/${r.id}`} className="block text-xs font-semibold truncate hover:text-[#E86A3A]">
          ...
          {(data?.recent ?? []).map((r) => (
            <Link key={r.id} href={`/reel/${r.id}`} className="rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition">
              ...
              <div className="p-2.5">
                ...
                <div className="text-xs text-gray-400">{r.planned_date}</div>
              </div>
            </Link>
          ))}
          {(data?.recent?.length ?? 0) === 0 && <div className="text-gray-400 col-span-4 py-8 text-center">아직 만든 릴스가 없습니다. ✨ 오늘의 릴스에서 시작해보세요.</div>}
        </div>
      </Card>
    </div>
  );
