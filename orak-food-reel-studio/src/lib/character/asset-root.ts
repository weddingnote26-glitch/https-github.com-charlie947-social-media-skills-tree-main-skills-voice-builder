...
export function masterMissingReason(a: OrakiAssets = orakiAssets()): string | null {
  ...
  const where = a.source === "설정 폴더"
    ? `설정한 에셋 폴더에서 마스터 이미지를 찾지 못했습니다.\n찾아본 곳: ${path.join(a.root, "master", "oraki_master.png")}`
    : `기본 에셋 폴더에서 마스터 이미지를 찾지 못했습니다.\n찾아본 곳: ${path.join(a.root, "character_sheet.png")}`;
