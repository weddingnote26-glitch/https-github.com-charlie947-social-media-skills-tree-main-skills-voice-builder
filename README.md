...
### `orak-shortform-studio/` — 오락 숏폼 AI 스튜디오 (개발 중)

Instagram 릴스 · YouTube 쇼츠용 **세로 영상(MP4)** 을 만드는 윈도우 프로그램입니다.
맛집 정보를 넣으면 대본 → 이미지 → 영상 → 음성 → 자막 → 합성까지 만들고, **업로드는 담당자가 직접** 합니다.

- 기술: Python + PySide6 (위 릴스 스튜디오와 **다른 프로그램**입니다)
- 오락이가 나오는 장면만 영상 AI 로 만들고, 음식·매장은 실제 사진을 움직여 비용을 줄입니다
- 대가·협찬을 받았으면 「유료광고 포함」이 시작·중간·끝에 **자동으로** 들어갑니다

**지금 상태** → Stage 2 (화면 뼈대) 까지. 버튼을 눌러도 아직 실제로 만들지는 않습니다.
자세한 건 `orak-shortform-studio/STAGE2_화면.md` 와 [WORKLOG.md](WORKLOG.md) 를 보세요.

> **당근 카드뉴스 프로그램과 폴더·설정·API 키를 공유하지 않습니다.** 별개 프로그램입니다.
